...
data$Lower_Bound <- s$Lower_Bound[match(data$Coh, Lower_Bound$Coh)]
data$Upper_Bound <- s$Upper_Bound[match(data$x, Upper_Bound$x)]
data$mean<- s$pcorr_mean[match(data$x, pcorr_mean$x)]
# confidence band
p1 <- p1 + 
  geom_line(data, aes(x, mean), size = 2) +
...
plotribbon(data=df.summary, group=Coh, y=Pct.Corr, y_lbl, title_text, 1)
 # I can try stat_smooth function next time
```  
