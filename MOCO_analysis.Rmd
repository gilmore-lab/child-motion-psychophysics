...
# from the plot, we can see the accuracy of subj 1088 has significantly below average of the sample group.
```  
# plot of RT by subj
```{r RT-subj-mean-plot}
# Plot theme, customizations
...
p.corr <- p.corr + 
  ...
  geom_ribbon(aes(ymin = lower_bound, ymax =upper_bound), fill = "grey70",
                alpha=0.5) 
p.corr
# we can see the small variance at the coherence level of .20, .40, .80 for small sample size. In radial condition, one subj (1088) is deviant in speed of 2 deg/s.
```
