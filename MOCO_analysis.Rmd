...
shapiro.test(df2$RT)  # From the output, the p-value > 0.05 implying that the distribution of the data are not significantly different from normal distribution. In other words, we can assume the normality.
```
...
```{r probit-without-Gender-Age, eval=FALSE}
...
```{r probit-without-gender, eval=FALSE}
...
```{r probit-without-age, eval=FALSE}
...
#### Empty model
```{r probit-empty, eval=FALSE}
form.empty.rt <- df2$RT~(1|df2$SubID)
model.empty.rt <- lmer(formula = form.empty.rt, data = df2, REML=FALSE,na.omit=TRUE)
