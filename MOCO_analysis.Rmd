...
p.acc.mean <-q <- data.frame(
  x   = seq(1, 100, 1),
  ts1 = sample(1:100),
  ts2 = sample(1:100))

q$mean <- apply(df.bysub.bycond, SubID, function(row) mean(row[-1]))
q$sd   <- apply(q, SubID, function(row) sd(row[-1]))

eb <- aes(ymax = mean + sd, ymin = mean - sd)
ggplot(data = q, aes(x = x, y = mean)) + 
  geom_line(size = 2) + 
  geom_ribbon(eb, alpha = 0.5) ggplot(data=df.bysub.bycond, aes(x=Coh, y=Pct.Corr)) +
    geom_line(data = df.bysub.bycond, aes(gp, y)) +
