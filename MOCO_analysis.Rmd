...
# from this boxplot, we can see two subject has larger variance than the other. (Shall we include these two subjects?)
```
...
boxplot(RT ~ SubID, data=df_clean, main="the boxplot of reaction time for each subject")

# In the previous aggregation, no response trials are treated as "False" response and RT is 0. Revalue this RT
df_clean$RT[df_clean$RT==0 & isFALSE(df_clean$Acc)] <- NA
boxplot(RT ~ SubID, data=df_clean, main="the boxplot of reaction time for each subject")
```
