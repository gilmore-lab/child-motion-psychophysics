...
foo <- boxplot(RT ~ SubID, data=df, main="the boxplot of reaction time for each subject")
...
df_clean <- df %>%
  group_by(SubID) %>%
  mutate(RT = outliersQ(RT))
boxplot(RT ~ SubID, data=df_clean, main="the boxplot of reaction time for each subject cleaned by quantile")
...
df2<-df.clean
df2$RT[which(df.clean$RT==0 & df.clean$Acc==FALSE)] <- NA
...
# df.summary %>% 
#   group_by(Coh) %>% 
#   mutate(pcorr.mean=mean(Pct.Corr, na.rm = T),
#         pcorr.sd=sd(Pct.Corr, na.rm = T)) %>%
#   ungroup() 
...
s<- ddply(df.summary, ~Coh, summarize,  pcorr_mean=mean(Pct.Corr),pcorr_sd=sd(Pct.Corr))
  # data[,list(mean=mean(y),sd=sd(y)),by=group]
  # do not work well in function
sd = 1
s$Lower_Bound <- s$pcorr_mean-sd*s$pcorr_sd
s$Upper_Bound <- s$pcorr_mean+sd*s$pcorr_sd
# df.summary$lower_Bound <- df.summary %>%
                          group_by(Coh) %>%
                          mutate(Lower_Bound = s$Lower_Bound)
df.summary$upper_Bound <- s$Upper_Bound[match(df.summary$Coh, df.summary$Coh)]
df.summary$lower_bound <- s$Lower_Bound[match(df.summary$Coh, df.summary$Coh)]
df.summary$pcorr.mean <- s$Upper_Bound[match(df.summary$Coh, df.summary$Coh)]
...
# p2 <- 
# xHigh <- Coh
# yHigh <- y + abs(rnorm(10,sd=3))
# xLow <- Coh
# yLow <- y - abs(rnorm(10,sd=3))
# p2 <- ggplot(df_ring, aes(x= timeNumeric, y= mean_ring)) + 
#        geom_line() + geom_point() + 
#        geom_errorbar(aes(ymin = mean_ring-sd_ring, ymax= mean_ring + sd_ring), 
#          width=.2, position=position_dodge(0.05))
# p3 <-arrows(xHigh,yHigh,xLow,yLow,col=2,angle=90,length=0.1,code=3)
p <- p + 
  geom_line(aes(group=SubID, color=AgeYrs)) +
...
# let me test
```
outliers<-outlierTest(model, cutoff=0.05, n.max=20, order=TRUE, 
